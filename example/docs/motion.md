...
当前版本说明：
- `move_cartesian()` 默认使用 `config/dh_parameters_config.json` 创建 IK/FK 模型
- 该配置会统一影响 SDK 中的位姿查询、视觉抓取、JoyCon 默认运动学与 MuJoCo 默认运动学
- 如需读取当前 SDK 实际使用的运动学配置，可通过 `HorizonArmSDK.get_kinematics_config()` 或 `load_kinematics_config()` 查询
...
kin_cfg = sdk.get_kinematics_config()  # 查看当前统一 DH / joint_offsets / joint_limits 配置
```

### 独立使用

```python
from Embodied_SDK import MotionSDK, load_kinematics_config
