...
这是一个用于控制ZDT闭环驱动板的Python SDK，通过ESP32+CAN硬件保护板进行控制，PC端使用UCP串口协议通信。
...
本文面向应用层开发者，底层协议细节省略。UCP模式下，ZDT协议封装在ESP32固件中，PC端通过串口与ESP32通信。
...
### 多机连接示例
```
电脑(COM31) ←→ ESP32+CAN硬件板 ←→ CAN总线 ←→ ZDT驱动板1(ID=1)
                                      ├→ ZDT驱动板2(ID=2)
                                      └→ ZDT驱动板N(ID=N)
```
