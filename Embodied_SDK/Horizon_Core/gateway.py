...
# -*- coding: utf-8 -*-
"""
Embodied_SDK.Horizon_Core 网关（统一出口）
================================
...
def get_control_core() -> Any:
    """
    获取运动控制核心模块 `Embodied_SDK.Horizon_Core.Control_SDK.Control_Core`
    ...
    """
    from Embodied_SDK.Horizon_Core.Control_SDK import Control_Core  # 延迟导入避免循环
    return Control_Core
...
def create_motor_controller(*args, **kwargs) -> Any:
    ...
    """
    from Embodied_SDK.Horizon_Core.Control_SDK.Control_Core import create_motor_controller as _create
...
def register_motor_driver(name: str, controller_cls: Any, 
    ...
    ```
    """
    from Embodied_SDK.Horizon_Core.Control_SDK.Control_Core import register_motor_driver as _register
...
def set_default_motor_driver(name: str) -> None:
    ...
    ```
    """
    from Embodied_SDK.Horizon_Core.Control_SDK.Control_Core import set_default_motor_driver as _set_default
...
def get_embodied_module() -> Any:
    """
    获取具身智能功能函数模块 `Embodied_SDK.Horizon_Core.core.embodied_core.embodied_func`
    ...
    用法示例：
        ...
        ok = embodied.c_a_j([...])
    """
    from Embodied_SDK.Horizon_Core.core.embodied_core import embodied_func  # 延迟导入
    return embodied_func


def get_embodied_internal_module() -> Any:
    """
    获取具身智能内部工具模块 `Embodied_SDK.Horizon_Core.core.embodied_core.embodied_internal`
    """
    from Embodied_SDK.Horizon_Core.core.embodied_core import embodied_internal  # 延迟导入
...
def get_hierarchical_decision_system_class() -> Any:
    ...
    用法示例：
        ...
        hds = HDS(config)
    """
    from Embodied_SDK.Horizon_Core.core.embodied_core.hierarchical_decision_system import HierarchicalDecisionSystem
...
def create_aisdk(*args, **kwargs) -> Any:
    ...
    """
    from Embodied_SDK.Horizon_Core.AI_SDK import AISDK  # 延迟导入
...
def create_depth_estimation_sdk(*args, **kwargs) -> Any:
    ...
    try:
        from Embodied_SDK.Horizon_Core.AI_SDK import DepthEstimationSDK
        return DepthEstimationSDK(*args, **kwargs)
    except ImportError:
        # 回退到底层类
        from Embodied_SDK.Horizon_Core.core.arm_core.Depth_Estimation import StereoDepthEstimator
